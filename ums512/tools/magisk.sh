...
if $magiskdata; then
  ...
  # Magisk v29.0 Use magiskinit directly
  cp -af $MAGISKBIN/magiskinit $MAGISKBIN/magisk
...
# Added support for 16k page size in v28.0
echo "PATCHVBMETAFLAG=false" >> config
# Set to handle 2SI devices properly (fixed in v28.1)
echo "TWOSTAGEINIT=true" >> config
