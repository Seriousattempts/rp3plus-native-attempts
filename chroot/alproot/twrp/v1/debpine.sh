...
# 2. Extract vulkan rootfs (armvulkbian64.tar.gz) to /vulkan in partition 77
#
...
VULK="${SD_CARD}/armvulkbian64.tar.gz"
...
echo "3. Extract armvulkbian64.tar.gz to /vulkan in partition root"
sleep 5

# ============================================================
# [1/9] CHECK REQUIRED FILES
# ============================================================
echo ""
...
if [ ! -f "$VULK" ]; then
    echo "ERROR: $VULK not found!"
    echo "Please ensure armvulkbian64.tar.gz is on your SD card at: $SD_CARD"
    exit 1
fi
echo "Found armvulkbian64.tar.gz"

# ============================================================
# [2/9] CHECK PARTITIONS
# ============================================================
echo ""
echo "[2/9] Checking partitions..."
...
# ============================================================
# [4/9] UNMOUNT PARTITION 77 IF MOUNTED
# ============================================================
echo ""
...
# ============================================================
# [6/9] FORMAT PARTITION 77 AS EXT4
# ============================================================
echo ""
...
# ============================================================
# [8/9] INSTALL ALPINE ROOTFS TO PARTITION ROOT
# ============================================================
echo ""
echo "[8/9] Installing Alpine rootfs to partition root..."
...
# ============================================================
# [9/9] EXTRACT VULKAN ROOTFS TO /vulkan IN PARTITION ROOT
# ============================================================
echo ""
echo "[9/9] Extracting vulkan rootfs to /vulkan..."

mkdir -p "${MOUNT_POINT}/vulkan" 2>/dev/null

echo "Extracting armvulkbian64.tar.gz to ${MOUNT_POINT}/vulkan ..."
echo "This may take several minutes depending on archive size."
echo "============================================"

tar -xpzf "$VULK" -C "${MOUNT_POINT}/vulkan" &
TAR_PID=$!

while kill -0 $TAR_PID 2>/dev/null; do
    sleep 10
    CURRENT_VULK_FILES=$(find "${MOUNT_POINT}/vulkan" -type f 2>/dev/null | wc -l)
    CURRENT_VULK_SIZE=$(du -sh "${MOUNT_POINT}/vulkan" 2>/dev/null | awk '{print $1}')
    echo "Extracting... $CURRENT_VULK_FILES files written - Current size: $CURRENT_VULK_SIZE"
done

wait $TAR_PID
VULK_EXTRACT_RESULT=$?

VULK_TOTAL_FILES=$(find "${MOUNT_POINT}/vulkan" -type f 2>/dev/null | wc -l)
VULK_DIR_SIZE=$(du -sh "${MOUNT_POINT}/vulkan" 2>/dev/null | awk '{print $1}')

echo "============================================"
if [ $VULK_EXTRACT_RESULT -ne 0 ]; then
    echo "WARNING: vulkan extraction reported errors (exit code: $VULK_EXTRACT_RESULT)"
    echo "Files extracted so far: $VULK_TOTAL_FILES"
else
    echo "vulkan extraction completed. Total vulkan files: $VULK_TOTAL_FILES"
fi
...
echo ""
echo "Verifying /vulkan (vulkan rootfs)..."
if [ -d "${MOUNT_POINT}/vulkan" ]; then
    echo "  /vulkan exists ($VULK_TOTAL_FILES files, $VULK_DIR_SIZE)"
    for dir in etc bin usr var home root; do
        if [ -d "${MOUNT_POINT}/vulkan/$dir" ]; then
            echo "    /vulkan/$dir exists"
        else
            echo "    /vulkan/$dir MISSING"
            ALL_DIRS_EXIST=false
        fi
    done
else
...
echo ""
echo "========================================"
if [ "$ALL_DIRS_EXIST" = true ] && [ $COPY_RESULT -eq 0 ] && [ $VULK_EXTRACT_RESULT -eq 0 ]; then
...
echo "- vulkan rootfs at:      /vulkan - $VULK_TOTAL_FILES files"
