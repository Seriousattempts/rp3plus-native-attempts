#!/bin/bash
# UNISOC Hardware Device Configuration Script for Debian
...
# === MODULE LOADING HELPER FOR DEBIAN ===
load_kernel_module() {
    local module_name="$1"
    local module_path="$2"
    ...
    if lsmod | grep -q "^$module_name "; then
        hw_log "  ✓ Module $module_name already loaded"
        return 0
    fi
    
    # Try modprobe first (standard Debian method)
    if modprobe "$module_name" 2>/dev/null; then
        hw_log "  ✓ Loaded module: $module_name (via modprobe)"
        return 0
    fi
    
    # Try insmod with Debian standard paths
    local kernel_ver=$(uname -r)
    local module_locations=(
        "$module_path"
        "/lib/modules/$kernel_ver/kernel/drivers/$module_path"
        "/lib/modules/$kernel_ver/extra/$module_name.ko"
        "/usr/lib/modules/$kernel_ver/kernel/drivers/$module_path"
        "/vendor/lib/modules/$module_name.ko"
    )
    
    for location in "${module_locations[@]}"; do
        if [ -f "$location" ]; then
            if insmod "$location" 2>/dev/null; then
                hw_log "  ✓ Loaded module: $module_name (via insmod from $location)"
                return 0
            fi
        fi
    done
    
    hw_log "  ⚠ Could not load module: $module_name"
    return 1
}
...
initialize_v4l2_cameras() {
    ...
    hw_log "  Loading camera kernel modules..."
    load_kernel_module "sprd_sensor" "media/platform/sprd-sensor/sprd_sensor.ko"
    load_kernel_module "sprd_cpp" "media/platform/sprd-cpp/sprd_cpp.ko"
    load_kernel_module "videodev" "media/v4l2-core/videodev.ko"
    ...
    # Method 1: Trigger camera sensor probe via sysfs
    if [ -f /sys/bus/platform/drivers/sprd-sensor/bind ]; then
        for dev in /sys/devices/platform/soc/soc:cam-subsys*; do
            if [ -d "$dev" ]; then
                basename "$dev" > /sys/bus/platform/drivers/sprd-sensor/bind 2>/dev/null
    ...
    if [ ! -e /dev/video0 ]; then
        ...
        # Get video device major number (typically 81)
        VIDEO_MAJOR=$(grep video /proc/devices | awk '{print $1}' | head -1)
        if [ -z "$VIDEO_MAJOR" ]; then
            VIDEO_MAJOR=81
        fi
        ...
        for i in $(seq 0 15); do
            if [ ! -e /dev/video$i ]; then
                mknod /dev/video$i c $VIDEO_MAJOR $i 2>/dev/null
                chmod 666 /dev/video$i 2>/dev/null
                hw_log "    Created: /dev/video$i (major: $VIDEO_MAJOR, minor: $i)"
            fi
        done
    fi
    
    # Create media controller devices
    if [ ! -e /dev/media0 ]; then
        hw_log "  Creating media controller devices..."
        
        # Get media device major number (typically 250 or dynamic)
        MEDIA_MAJOR=$(grep media /proc/devices | awk '{print $1}' | head -1)
        if [ -z "$MEDIA_MAJOR" ]; then
            MEDIA_MAJOR=250
        fi
        
        for i in $(seq 0 3); do
            if [ ! -e /dev/media$i ]; then
                mknod /dev/media$i c $MEDIA_MAJOR $i 2>/dev/null
                chmod 666 /dev/media$i 2>/dev/null
                hw_log "    Created: /dev/media$i (major: $MEDIA_MAJOR, minor: $i)"
            fi
        done
    fi
    ...
    if [ ! -e /dev/v4l-subdev0 ]; then
        ...
        # V4L subdev uses video major, minor starts at 128
        VIDEO_MAJOR=$(grep video /proc/devices | awk '{print $1}' | head -1)
        if [ -z "$VIDEO_MAJOR" ]; then
            VIDEO_MAJOR=81
        fi
        ...
        for i in $(seq 0 7); do
            ...
            if [ ! -e /dev/v4l-subdev$i ]; then
                mknod /dev/v4l-subdev$i c $VIDEO_MAJOR $minor 2>/dev/null
                chmod 666 /dev/v4l-subdev$i 2>/dev/null
                hw_log "    Created: /dev/v4l-subdev$i (major: $VIDEO_MAJOR, minor: $minor)"
            fi
        done
    fi
    
    # Trigger udev to update device nodes if running
    if command -v udevadm >/dev/null 2>&1; then
        hw_log "  Triggering udev device update..."
    ...
    for gpio in $CAMERA_GPIOS; do
        if [ ! -d /sys/class/gpio/gpio$gpio ]; then
            if echo $gpio > /sys/class/gpio/export 2>/dev/null; then
                ...
                hw_log "  ⚠ Failed to export GPIO $gpio (may already be in use)"
            fi
        else
            hw_log "  ✓ GPIO $gpio already exported"
        fi
    done
...
initialize_dcam() {
    ...
    DCAM_FOUND=0
    for path in /sys/devices/platform/soc/soc:cam-subsys/dcam* \
                /sys/devices/platform/dcam* \
                /sys/module/sprd_sensor/parameters; do
        if [ -e "$path" ]; then
    ...
    if [ $DCAM_FOUND -eq 0 ]; then
        hw_log "  ⚠ No DCAM paths found"
    fi
    
    # Enable DCAM if control file exists
    for enable_file in /sys/devices/platform/soc/soc:cam-subsys/dcam_enable \
                       /sys/devices/platform/dcam*/enable; do
        if [ -f "$enable_file" ]; then
            echo 1 > "$enable_file" 2>/dev/null
            hw_log "  ✓ Enabled DCAM controller via $enable_file"
        fi
    done
...
configure_device "/dev/dri/renderD128"
