...
# Create TWRP start script for /external_sd/
cat > start.sh <<- 'TWRPEOF'
#!/system/bin/sh
# Ubuntu in TWRP Launcher for F2FS SD Card
# TWRP automatically mounts SD card at /external_sd/
...
if [ ! -d "$INSTALL_PATH/ubuntu-fs" ]; then
    ...
    echo "Looking for ubuntu-fs directory..."
...
# Create essential device nodes manually
mknod -m 666 ubuntu-fs/dev/null c 1 3 2>/dev/null || echo "/dev/null exists"
mknod -m 666 ubuntu-fs/dev/zero c 1 5 2>/dev/null || echo "/dev/zero exists"
mknod -m 666 ubuntu-fs/dev/full c 1 7 2>/dev/null || echo "/dev/full exists"
mknod -m 666 ubuntu-fs/dev/random c 1 8 2>/dev/null || echo "/dev/random exists"
mknod -m 666 ubuntu-fs/dev/urandom c 1 9 2>/dev/null || echo "/dev/urandom exists"
mknod -m 666 ubuntu-fs/dev/tty c 5 0 2>/dev/null || echo "/dev/tty exists"

# Bind mount filesystems
echo "Mounting filesystems..."
mount -t proc proc ubuntu-fs/proc 2>/dev/null
mount -t sysfs sysfs ubuntu-fs/sys 2>/dev/null
mount -t devpts devpts ubuntu-fs/dev/pts 2>/dev/null

# Bind mount real /dev as backup
mount --bind /dev ubuntu-fs/dev 2>/dev/null
...
printf "\x1b[38;5;214m[${time1}]\e[0m \x1b[38;5;83m[Installer thread/INFO]:\e[0m \x1b[38;5;87m TWRP launcher: /external_sd/3PLUSLINUX/start.sh\n"
